...
const sf::Uint8 Chat::m_maxBufferLength = 100u; //change to Uint16/32 if needing more than 255
...
//***public functions***//
void Chat::Update(const sf::Event& event)
{
	...
	if(m_allowInput) //set to allow input by pressing m_chatKeyFirst
	{
		...
		if(event.type == sf::Event::KeyPressed)
		{
			...
			case sf::Keyboard::Return:
				if(m_lastKey != sf::Keyboard::Return && m_inputBuffer.size() > m_inputBufferPrefix.size()) //don't send 'empty' strings
				{
		...
		else if(event.type == sf::Event::KeyReleased && event.key.code == m_lastKey)
		{
			m_lastKey = sf::Keyboard::Unknown;
			return;
		}
		//else append new char
		else if (event.type == sf::Event::TextEntered && m_lastKey == sf::Keyboard::Unknown)
		{
			// Handle ASCII characters only, skipping backspace and delete
			if(event.text.unicode > 31 && event.text.unicode < 127 && m_inputBuffer.size() < m_maxBufferLength)
			{
...
const std::deque<TextLine>& Chat::GetOutputBuffer(void) const
{
...
bool Chat::WaitingInput(void) const
{
	return m_allowInput;
}

const ChatState& Chat::GetCurrentState(void) const
{
	return m_currentState;
}

void Chat::SetUserName(const std::string& name)
{
	sf::Lock lock(m_mutex);
...
void Chat::SetMotd(const std::string& string)
{
	m_motd = string;
}

void Chat::ExecIrcCmd(const std::string& cmd)
{
...
//***private functions***//
void Chat::m_OutputString(const std::string& str, sf::Color colour)
{	
...
std::string Chat::m_ParseCommand(const std::string& str)
{
	...
	else if(ENABLE_PLAY && cmd == "/play")
	{
		if(str.size() < cmd.size() + 1) return "";

		sf::Lock lock(m_mutex);
		m_soundRequest = str.substr(str.find_first_of(' ') + 1);
		return "silent";
	}
	else
...
void Chat::m_ProcessInput(void)
{
	...
	if(sendStr.front() == '/')
	{
		...
		{
			if(m_currentState != ChatServer) //don't print if server or the message will be sent to all clients
